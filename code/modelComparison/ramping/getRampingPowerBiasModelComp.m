function [WAIC, DIC, l_like, trial_WAIC, trial_DIC, trial_l_like, sample_likelihoods, sample_trial_likelihoods ] = getRampingTrialCompLog1PPowerBias(RampSamples,params,modelFit,timeSeries,pow)
...
[l_like,trial_l_like] = kcRampLikelihoodPowerBias(gpu_y, gpu_trIndex, gpu_trBetaIndex, betas, w2s, l_0, gamma, timeSeries.delta_t, params.DIC.meanLikelihoodSamples, bias,pow);
...
for ss = 1:thinRate:params.MCMC.nSamples
    ...
    [sample_likelihoods(ss), sample_trial_likelihoods(:,ss)] = kcRampLikelihoodPowerBias(gpu_y, gpu_trIndex, gpu_trBetaIndex, RampSamples.betas(ss+burnIn,:), RampSamples.w2s(ss+burnIn,1), RampSamples.l_0(ss+burnIn,:), RampSamples.gammas(ss+burnIn), timeSeries.delta_t, params.DIC.likelihoodSamples, RampSamples.biases(ss+burnIn,1),pow);
end
