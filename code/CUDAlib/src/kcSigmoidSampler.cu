...
__device__ KC_FP_TYPE dh_c(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE bias,KC_FP_TYPE steep,KC_FP_TYPE center) {
    KC_FP_TYPE num = -gamma*(steep)*KC_MAX(KC_MIN(KC_MAXN,KC_EXP(-steep*(lambda-center))),KC_MINN);
    KC_FP_TYPE denom = KC_MAX(KC_MIN(KC_MAXN,( 1 + KC_EXP(-steep*(lambda-center)) )*( 1 + KC_EXP(-steep*(lambda-center)) )),KC_MINN);
    return num/denom;
}
    ...
    if(idx < NT) {
        ...
        for(int ii = mBlkIdx[idx]; ii < mBlkIdx[idx+1]; ii++)  {
            ...
            if(jj == 0) {
                ...
                dr1 = dh_c(trueLambda,g,bias,steep,center);
            }
            ...
            trialSum[idx+jj*NT] += (y[ii]/r-dt)*dr1;
                ...
                for(int kk = jj; kk < NP; kk++) {
                    ...
                    if(kk == 0) {
                        ...
                        dr2 = dh_c(trueLambda,g,bias,steep,center);
                    }
