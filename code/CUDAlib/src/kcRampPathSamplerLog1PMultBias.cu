...
__device__ KC_FP_TYPE h(KC_FP_TYPE z, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE sh, KC_FP_TYPE bias) {
    return KC_MAX(MIN_P,KC_MIN((log1p(exp(z*gamma))*exp(sh)+bias)*dt,MAX_P));
}
...
__global__ void kcMoveParticles(KC_FP_TYPE * y, KC_FP_TYPE * spe, KC_FP_TYPE * pos, KC_FP_TYPE * wt,  KC_FP_TYPE * b, int * betaIdxVector, KC_FP_TYPE l_0, KC_FP_TYPE g, KC_FP_TYPE w, KC_FP_TYPE dt, KC_FP_TYPE * randN, KC_FP_TYPE sigMult, KC_FP_TYPE * log_li, KC_FP_TYPE * lw, KC_FP_TYPE * lw2, KC_FP_TYPE * ncdf, KC_FP_TYPE * posc, int * trIdx, int NT, int TT, int numParticles, int t, KC_FP_TYPE bias) {
    ...
    if(tr_num < NT) {
        ...
        if(t < trLength) {
            ...
            log_li[pidx]  = -h(pos[idx],g,dt,spe[row],bias)+y[row]*(log(fmax(h(pos[idx],g,1.0,spe[row],bias),1e-30))+log(dt))-lgamma(y[row]+1);
...
__global__ void kcSetupLG(KC_FP_TYPE * y,KC_FP_TYPE * spe,KC_FP_TYPE * lg,KC_FP_TYPE g, KC_FP_TYPE dt,int TT, KC_FP_TYPE bias) {
    ...
    if(idx < TT) {
        lg[idx] = exp( -h(1,g, dt,spe[idx],bias) + y[idx]*log(fmax(h(1,g,dt,spe[idx],bias),1e-30)) - lgamma(y[idx]+1));
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    //__global__ void kcSetupLG(KC_FP_TYPE * y,KC_FP_TYPE * spe,KC_FP_TYPE * lg,KC_FP_TYPE g, KC_FP_TYPE dt,int TT, KC_FP_TYPE bias) {
    kcSetupLG <<< nBlocksT, blockSizeT >>> (y,spe,lg,g,dt,TT,bias);
    ce = cudaDeviceSynchronize();
    if(ce != cudaSuccess) {
    ...
    for (int ii = 0; ii < maxTrialLength;ii++) {
        ...
        kcMoveParticles <<< nBlocks, blockSize >>> (y,spe,pos,wt, b_gpu,betaIdxVector,l_0,g,w,dt,randN, sigMult,log_li,lw,lw2,ncdf, posc, trIdx, NT, TT, numParticles, ii, bias);
        ce = cudaDeviceSynchronize();
        if(ce != cudaSuccess) {
            int currDev;
            cudaGetDevice(&currDev);
