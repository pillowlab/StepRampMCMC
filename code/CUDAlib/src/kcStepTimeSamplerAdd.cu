...
//}
__device__ KC_FP_TYPE log_p_y(KC_FP_TYPE y, KC_FP_TYPE rate, KC_FP_TYPE dt, KC_FP_TYPE sh) {
    KC_FP_TYPE logex = KC_MAX(KC_MINN,KC_MIN(log1p(exp(rate + sh)),KC_MAXN)); 
 ...
 */
__global__ void kcSampleSMStates(KC_FP_TYPE * z, KC_FP_TYPE * s, KC_FP_TYPE * sampleStats, KC_FP_TYPE * y, int * trialIndex, int * trialCoh, int NT, KC_FP_TYPE * alphas, KC_FP_TYPE * phi, KC_FP_TYPE delta_t, int maxJump, KC_FP_TYPE * randU, KC_FP_TYPE * nbPDF, KC_FP_TYPE * jumpToProbs, KC_FP_TYPE * spe) {
    ...
    if(idx < NT) {
        ...
        for(int ii = T-1; ii >= 0; ii--) {
            ...
            jumpToProbs[jumpT1_2+ii] = ((ii < T-1)?(jumpToProbs[jumpT1_2+ii+1]):(0)) + log_p_y(y[T1+ii],alphas[1],delta_t,spe[T1+ii]) ;
            jumpToProbs[jumpT1_3+ii] = ((ii < T-1)?(jumpToProbs[jumpT1_3+ii+1]):(0)) + log_p_y(y[T1+ii],alphas[2],delta_t,spe[T1+ii]) ;
        }
        ...
        for(int ii = 0; ii < maxJump; ii++) {
            ...
            if(ii < T) {
                KC_FP_TYPE p_y_init = log_p_y(y[T1+ii],alphas[0],delta_t,spe[T1+ii]);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    //sample the states
    kcSampleSMStates<<<NT,1>>>(z, s, sampleStats, y, trIndex, cohIndex, NT, alphas,  phi, dt, maxJump, randU, nbPDF, calculationSpace, spe);
