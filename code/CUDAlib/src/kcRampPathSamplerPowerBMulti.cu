    ...
    if(tr_num < NT) {
        ...
        if(t < trLength) {
            ...
            for(int nn = 0; nn < numNeur; nn++) {
                log_li[pidx] += -h(pos[idx],g[nn],dt,spe[row+TT*nn],bias[nn],log_power)+y[row+TT*nn]*(log(fmax(h(pos[idx],g[nn],1.0,spe[row+TT*nn],bias[nn],log_power),1e-30))+log(dt))-lgamma(y[row+TT*nn]+1);
            }
...
__global__ void kcSetupLG(KC_FP_TYPE * y,KC_FP_TYPE * spe,KC_FP_TYPE * lg,KC_FP_TYPE * g, KC_FP_TYPE dt,int TT, int numNeur, KC_FP_TYPE * bias, KC_FP_TYPE log_power) {
    int idx = blockIdx.x*blockDim.x + threadIdx.x;
    if(idx < TT) {
        KC_FP_TYPE log_sum  = 0; 
        for(int nn = 0; nn < numNeur; nn++) {
            log_sum += -h(1,g[nn], dt,spe[idx+TT*nn],bias[nn],log_power) + y[idx+TT*nn]*log(fmax(h(1,g[nn],dt,spe[idx+TT*nn],bias[nn],log_power),1e-30)) - lgamma(y[idx+TT*nn]+1);
        }
        lg[idx] = exp(log_sum);
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    KC_FP_TYPE  log_power     = mxGetScalar(prhs[19]);
    ...
    kcSetupLG <<< nBlocksT, blockSizeT >>> (y,spe,lg,g,dt,TT,numNeur,bias,log_power);
    ce = cudaDeviceSynchronize();
    if(ce != cudaSuccess) {
    ...
    for (int ii = 0; ii < maxTrialLength;ii++) {
        ...
        kcMoveParticles <<< nBlocks, blockSize >>> (y,spe,pos,wt, b_gpu,betaIdxVector,l_0,g,w,dt,randN, sigMult,log_li,lw,lw2,ncdf, posc, trIdx, NT, TT, numParticles, ii, numNeur, bias, log_power);
        ce = cudaDeviceSynchronize();
        if(ce != cudaSuccess) {
            int currDev;
            cudaGetDevice(&currDev);
