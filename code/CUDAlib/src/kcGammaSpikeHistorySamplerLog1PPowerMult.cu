...
__device__ KC_FP_TYPE h(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE sh, KC_FP_TYPE log_power) {
    KC_FP_TYPE logex = KC_MAX(KC_MINN,(gamma*lambda>100)?(gamma*lambda):KC_MIN(log1p(exp(lambda*gamma)),KC_MAXN));
    return KC_MIN(KC_POW(logex*1.00000,log_power*1.00)*KC_EXP(sh)*dt,KC_MAXN);
}

__device__ KC_FP_TYPE dh(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE sh, KC_FP_TYPE yh, KC_FP_TYPE log_power) {
    KC_FP_TYPE logex = KC_MAX(KC_MINN,(gamma*lambda>100)?(gamma*lambda):KC_MIN(log1p(exp(lambda*gamma)),KC_MAXN));
...
__global__ void kcBoundaryLikelihoodTrialHist(KC_FP_TYPE * y, KC_FP_TYPE * spe, KC_FP_TYPE * lambdas, int * crossingTimes, int * mBlkIdx, KC_FP_TYPE g, KC_FP_TYPE dt, int NT, KC_FP_TYPE * llSum, KC_FP_TYPE * trialSum, KC_FP_TYPE * trialSumRiemann, KC_FP_TYPE * h_filt, KC_FP_TYPE * y_hist, int NH, KC_FP_TYPE log_power) {
    ...
    if(idx < NT) {
        ...
        for(int ii = mBlkIdx[idx]; ii < mBlkIdx[idx+1]; ii++)  {
            ...
            KC_FP_TYPE r = h(trueLambda,g,1,sh,log_power);
	    ...
	    for(int jj = 0; jj < NH+1 ; jj++) {
            ...
            if(jj < NH && ii<(mBlkIdx[idx]+jj+1)) {
                ...
                yh1 = trueLambda;
                dr = dh(trueLambda,g,1,sh,yh1,log_power);
            }
            		...
            		// in that case, we want the ii - jj spike of the y history
               	    if(kk < NH && ii<(mBlkIdx[idx]+kk+1)) {
                        ...
                        dr2 = r*yh2;
	   	       		   	...
	   	       		   	yh2 = trueLambda;
                        dr2 = dh(trueLambda,g,1,sh,yh2,log_power);
		       		}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    KC_FP_TYPE  log_power     = mxGetScalar(prhs[11]);
    ...
    kcBoundaryLikelihoodTrialHist<<< numBlocks,blockSize >>>(y,spe,lambda,crossingTimes,trIdx,g,dt, NT,log_p_y,der_log_p_y,G_log_p_y1,h_filt,y_hist,NH,log_power);
