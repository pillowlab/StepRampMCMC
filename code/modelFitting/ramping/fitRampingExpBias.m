...
function [ RampFit, RampSamples, LatentDataHandler] = fitRampingExpBias(timeSeries,params)
...
[log_p_lambda, der_log_p_y, G_log_p_y, der_log_p_y_bias, G_log_p_y_bias, G_off_diag] = kcRampBoundHeightSamplerExpBias(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,RampSamples.gammas(ss-1),timeSeries.delta_t,G_prior,der_log_prior,RampSamples.biases(ss-1),bias_g,bias_l);
...
[log_p_lambda_star, der_log_p_y_star, G_log_p_y_star, der_log_p_y_bias_star, G_log_p_y_bias_star, G_off_diag_star] = kcRampBoundHeightSamplerExpBias(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,gamma_star,timeSeries.delta_t,G_prior_star,der_log_prior_star,bias_star,bias_g_star,bias_l_star);
...
p_sig_star = (g_delta^2)*inv(G_star);
    ...
    if ss > params.MCMC.burnIn && mod(ss-params.MCMC.burnIn-1,params.MCMC.thinRate)==0;                                                                                                         
        ...
        fr = exp(min(lambdaN,1)*RampSamples.gammas(ss))+RampSamples.biases(ss);                                                                                              
