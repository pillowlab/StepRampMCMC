...
params.rampPrior.beta_mu    = 0;  
params.rampPrior.beta_sigma = 0.1; % p(\beta_c) = \frac{1}{2*beta_sigma^2} \exp(-\frac{1}{2*beta_sigma^2} (\beta_c - beta_mu))

%diffusion variance (\omega^2)   inverse gamma distribution
params.rampPrior.w2_shape = 0.02;
params.rampPrior.w2_scale = 0.02; %p(\omega^2) = \frac{w2_scale^w2_shape}{\Gamma(w2_shape)} (\omega^2)^{-w2_shape-1} \exp(-\frac{w2_scale}{\omega^2})

%initial level (l_0)  truncated normal distribution
params.rampPrior.l0_mu    = 0;
params.rampPrior.l0_sigma = 10; %p(l_0) \propto \frac{1}{2*l0_sigma^2} \exp(-\frac{1}{2*l0_sigma^2} (l_0 - l0_mu)), given l_0 < 1

%firing rate multiplier (\gamma)  gamma distribution
params.rampPrior.gammaAlpha = 2;  
params.rampPrior.gammaBeta  = 0.05; %p(\gamma) = \frac{gammaBeta^{gammaAlpha} \gamma^{gammaAlpha-1} \exp(-\gamma*gammaBeta)}{\Gamma(gammaAlpha)}

%% stepping model prior

%firing rate (\alpha_s; same prior for each state) gamma distribution
params.stepPrior.alpha.rate  = 1;
params.stepPrior.alpha.shape = 1; %p(\alpha_s) = \frac{alpha.rate^{alpha.shape} \alpha_s^{alpha.shape-1} \exp(-\alpha_s*alpha.rate)}{\Gamma(alpha.shape)}

%step time distribution - ``number of failures'' (r) gamma distribution
params.stepPrior.negBin.failShape  = 2;
params.stepPrior.negBin.failRate   = 1; %p(r) = \frac{negBin.failRate^{negBin.failShape} r^{negBin.failShape-1} \exp(-r*negBin.failRate)}{\Gamma(negBin.failShape)}

%step time distribution - ``success probability''  (p_c; coherence dependent) beta distribution
params.stepPrior.negBin.succBeta   = 1;
params.stepPrior.negBin.succAlpha  = 1; % p(p_c)  = p_c^{negBin.succAlpha-1} (1-p_c)^{negBin.succBeta-1) / B(negBin.succAlpha, negBin.succBeta)

%probability of up step (\phi_c; coherence dependent) beta distribution
params.stepPrior.switchto.alpha    = 1;
params.stepPrior.switchto.beta     = 1; % p(\phi_c)  = \phi_c^{switchto.alpha-1} (1-\phi_c)^{switchto.beta-1) / B(switchto.alpha, switchto.beta)
