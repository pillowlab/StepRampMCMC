...
__device__ KC_FP_TYPE h(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE bias, KC_FP_TYPE log_power) {
    KC_FP_TYPE logex =  KC_MAX(KC_MINN,(gamma*lambda>100)?(gamma*lambda):KC_MIN(log1p(exp(lambda*gamma)),KC_MAXN));
    return KC_MIN((KC_POW(logex*1.00000,log_power)+bias)*dt,KC_MAXN);
}

__device__ KC_FP_TYPE dh(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE log_power) {
    KC_FP_TYPE logex = KC_MAX(KC_MINN,(gamma*lambda>100)?(gamma*lambda):KC_MIN(log1p(exp(lambda*gamma)),KC_MAXN));
    KC_FP_TYPE log_der = lambda/(1+KC_MAX(exp(-lambda*gamma),KC_MINN));
...
__global__ void kcBoundaryLikelihoodTrial(KC_FP_TYPE * y, KC_FP_TYPE * lambdas, int * crossingTimes, int * mBlkIdx, KC_FP_TYPE g, KC_FP_TYPE dt, int NT, KC_FP_TYPE * llSum, KC_FP_TYPE * trialSum, KC_FP_TYPE * trialSumRiemann, KC_FP_TYPE bias, KC_FP_TYPE * trialSumB, KC_FP_TYPE * trialSumRiemannB, KC_FP_TYPE * trialRiemannOffDiag, KC_FP_TYPE log_power) {
    ...
    if(idx < NT) {
        ...
        for(int ii = mBlkIdx[idx]; ii < mBlkIdx[idx+1]; ii++)  {
            ...
            KC_FP_TYPE r  = h(trueLambda,g,1,bias,log_power);
            llSum[idx] += y[ii]*(KC_LOG(r)+KC_LOG(dt)) - dt*r -lgamma(y[ii]+1);
            
            KC_FP_TYPE dr = dh(trueLambda,g,1,log_power);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    kcBoundaryLikelihoodTrial<<< numBlocks,blockSize >>>(y,lambda,crossingTimes,trIdx,g,dt, NT,log_p_y,der_log_p_y,G_log_p_y1,bias,der_log_p_y_bias,G_log_p_y1_bias,G_off_diag,log_power);
