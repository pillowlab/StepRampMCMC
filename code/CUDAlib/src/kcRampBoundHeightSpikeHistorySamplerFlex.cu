...
__global__ void kcSumLangevinVars(KC_FP_TYPE * der, KC_FP_TYPE * der_sum, KC_FP_TYPE * G, KC_FP_TYPE * G_sum, KC_FP_TYPE * ll, KC_FP_TYPE * ll_sum, int * mBlkIdx, int NT, int NH, KC_FP_TYPE * gPrior, KC_FP_TYPE * lPrior) {
    ...
    if(nsum == 0) {
        ...
        for(int idx = 0; idx < NH+1; idx++) {
            for(int idx2 = 0; idx2 < NH+1; idx2++) {
                    G_sum[idx*(NH+1)+idx2] = 0;
                    G_sum[idx*(NH+1)+idx2] = gPrior[idx*(NH+1)+idx2];
            }
        }
        for(int idx = 0; idx < NT; idx++) {
      	    for(int idx2 = 0; idx2 < NH+1; idx2++) {
                for(int idx3 =0; idx3 < NH+1; idx3++) {
                    G_sum[idx2*(NH+1)+idx3] -= G[idx+(idx2*(NH+1)+idx3)*NT];
                }
            }
        }
...
//derivates of  firing rate function w.r.t. gamma and history filters (assuming fixed latent variables)
__device__ KC_FP_TYPE h(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE modelInd, KC_FP_TYPE sh) {
    if (modelInd > 1e-10) {
        return KC_MIN(KC_POW(log(1.0+exp(lambda*gamma+sh)),modelInd)*dt,KC_MAXN);
    }
    else {
        return KC_MIN(exp(lambda*gamma+sh)*dt,KC_MAXN);
    }
}

__device__ KC_FP_TYPE dh(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE modelInd, KC_FP_TYPE sh, KC_FP_TYPE mult) {
    if (modelInd > 1e-10) {
        KC_FP_TYPE logex = KC_MIN(log(1.0+exp(lambda*gamma+sh)),KC_MAXN);
        KC_FP_TYPE log_der = KC_MIN(mult/(1+exp(-lambda*gamma-sh)),KC_MAXN);
        KC_FP_TYPE der = modelInd*KC_POW(logex,modelInd-1)*log_der;
        return KC_MIN(der*dt,KC_MAXN);
    }
    else {
        return KC_MIN(dt*mult*KC_EXP(gamma*lambda+sh),KC_MAXN);
    }    
}


// computes log p(single trial | gamma, fixed lambdas, spike history)
__global__ void kcBoundaryLikelihoodTrialHist(KC_FP_TYPE * y, KC_FP_TYPE * spe, KC_FP_TYPE * lambdas, int * crossingTimes, int * mBlkIdx, KC_FP_TYPE g, KC_FP_TYPE dt, int NT, KC_FP_TYPE * llSum, KC_FP_TYPE * trialSum, KC_FP_TYPE * trialSumRiemann, KC_FP_TYPE * h_filt, KC_FP_TYPE * y_hist, int NH, KC_FP_TYPE modelInd) {
    ...
    if(idx < NT) {
        ...
        for(int ii = mBlkIdx[idx]; ii < mBlkIdx[idx+1]; ii++)  {
            ...
            KC_FP_TYPE r    = KC_MAX(KC_MINN,h(trueLambda,g,1,modelInd,sh));
	    ...
	    for(int jj = 0; jj < NH+1 ; jj++) {
            ...
            KC_FP_TYPE dr = dh(trueLambda,g,1,modelInd,sh,yh1);
            trialSum[idx+jj*NT] += (y[ii]/r-dt)*dr;

                //for(int kk = jj+1; kk < NH+1; kk++) 
                for(int kk = 0; kk < NH+1; kk++) {
                    KC_FP_TYPE yh2 = 0;
            		// if index is one of the first NH indices of y, the spike history depends on spikes in the time before the analyzed spike train y
            		// in that case, we want the ii - jj spike of the y history
				    ...
				    KC_FP_TYPE dr2 = dh(trueLambda,g,1,modelInd,sh,yh2);
            		trialSumRiemann[idx+(jj*(NH+1)+kk)*NT] += KC_MIN(-1*dt*dr*dr2/r,KC_MAXN);
        		}
            }
        }
    }
...
//  11 = modelInd (power if use log1p transfer function, 0 if using exp)
//
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    KC_FP_TYPE modelInd = mxGetScalar(prhs[11]);
    ...
    kcBoundaryLikelihoodTrialHist<<< numBlocks,blockSize >>>(y,spe,lambda,crossingTimes,trIdx,g,dt, NT,log_p_y,der_log_p_y,G_log_p_y1,h_filt,y_hist,NH,modelInd);
