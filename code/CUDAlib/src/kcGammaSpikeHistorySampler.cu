...
__global__ void kcSumLangevinVars(KC_FP_TYPE * der, KC_FP_TYPE * der_sum, KC_FP_TYPE * G, KC_FP_TYPE * G_sum, KC_FP_TYPE * ll, KC_FP_TYPE * ll_sum, int * mBlkIdx, int NT, int NH, KC_FP_TYPE * gPrior, KC_FP_TYPE * lPrior) {
    ...
    if(nsum == 0) {
        for(int idx = 0; idx < NH+1; idx++){
            der_sum[idx]=lPrior[idx];
        }
        for(int jj = 0; jj < NH+1; jj++){
        ...
        for(int idx = 0; idx < NH+1; idx++) {
            for(int idx2 = 0; idx2 < NH+1; idx2++) {
                    G_sum[idx+idx2*(NH+1)] = 0;
                    G_sum[idx+idx2*(NH+1)] = gPrior[idx*(NH+1)+idx2];
            }
        }
        for(int jj = 0; jj < NH+1; jj++) {
      	    for(int kk = 0; kk < NH+1; kk++) {
                for(int idx =0; idx < NT; idx++) {
                    G_sum[jj*(NH+1)+kk] -= G[idx+(jj*(NH+1)+kk)*NT];
                }
            }
        }
    }
...
__device__ KC_FP_TYPE h(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE sh) {
    KC_FP_TYPE fr = ((gamma*lambda+sh)>40)?(dt*(gamma*lambda+sh)):(dt*log1p(KC_EXP(gamma*lambda+sh)));
    return fr;
}

__device__ KC_FP_TYPE dh(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE sh, KC_FP_TYPE yh) {
    return dt*yh/(1+KC_MIN(KC_MAXN,KC_MAX(KC_MINN,KC_EXP(-1*gamma*lambda-sh))));
    //return dt*yh/(1+KC_EXP(-gamma*lambda-sh));
}


__device__ KC_FP_TYPE dh2_h(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE sh, KC_FP_TYPE yh, KC_FP_TYPE yh2) {
    KC_FP_TYPE ex  = KC_EXP(gamma*lambda+sh);
    //KC_FP_TYPE nex = 1/ex;
    KC_FP_TYPE nex = KC_EXP(-gamma*lambda-sh);
    KC_FP_TYPE lex = ((gamma*lambda+sh)>40)?(gamma*lambda+sh):KC_MAX(KC_LOG(1+ex),KC_MINN);
    //return (yh*yh2)*dt*ex/KC_MAX(KC_MINN,((ex+2+nex)*lex));
    return (yh*yh2)*dt*1/(1+nex)*1/(1+nex)/lex;
}


// computes log p(single trial | gamma, fixed lambdas, spike history)
__global__ void kcBoundaryLikelihoodTrialHist(KC_FP_TYPE * y, KC_FP_TYPE * spe, KC_FP_TYPE * lambdas, int * crossingTimes, int * mBlkIdx, KC_FP_TYPE g, KC_FP_TYPE dt, int NT, KC_FP_TYPE * llSum, KC_FP_TYPE * trialSum, KC_FP_TYPE * trialSumRiemann, KC_FP_TYPE * h_filt, KC_FP_TYPE * y_hist, int NH) {
    ...
    if(idx < NT) {
        ...
        for(int jj = 0; jj<NH+1; jj++){
            ...
            for(int kk = 0; kk<NH+1; kk++){
                trialSumRiemann[idx+(jj*(NH+1)+kk)*NT]=0;
            }
        }
        llSum[idx] = 0;        
        ...
        for(int ii = mBlkIdx[idx]; ii < mBlkIdx[idx+1]; ii++)  {
            ...
            KC_FP_TYPE ex    = KC_MIN(KC_EXP( g*trueLambda + sh),KC_MAXN);
            KC_FP_TYPE nex   = KC_MAX(KC_EXP(-g*trueLambda - sh),KC_MINN);
            KC_FP_TYPE logex = ((g*trueLambda+sh)<80)?(KC_MAX(log1p(ex),KC_MINN)):(g*trueLambda+sh);
            llSum[idx] += y[ii]*(KC_LOG(logex)+KC_LOG(dt)) - dt*logex - KC_GAMMALN(y[ii]+1.0);
            //KC_FP_TYPE r  = KC_MAX(KC_MINN,h(trueLambda,g,1,sh));
            KC_FP_TYPE r = logex;
	    ...
	    for(int jj = 0; jj < NH+1 ; jj++) {
            ...
            KC_FP_TYPE dr = dh(trueLambda,g,1,sh,yh1);
            //trialSumRiemann[idx+(jj*(NH+1)+jj)*NT] += -1*dh2_h(trueLambda,g,dt,sh,yh1,yh1);
            //trialSumRiemann[idx+(jj*(NH+1)+jj)*NT] += -1*dt*dr*dr/r;

            trialSum[idx+jj*NT] += (y[ii]/r-dt)*dr;

                for(int kk = jj; kk < NH+1; kk++) {
                //for(int kk = 0; kk < NH+1; kk++) {
				    ...
				    KC_FP_TYPE dr2 = dh(trueLambda,g,1,sh,yh2);
            		trialSumRiemann[idx+(NH+1)*NT*jj+NT*kk] += -1*dt*dr*dr2/r;
        		}
            }
        }
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    KC_FP_TYPE * gPrior;
    checkCudaErrors(cudaMalloc((void**)&gPrior,sizeof(KC_FP_TYPE)*(NH+1)*(NH+1)));
    checkCudaErrors(cudaMemcpy(gPrior,(KC_FP_TYPE*)mxGetPr(prhs[7]),sizeof(KC_FP_TYPE)*((NH+1)*(NH+1)),cudaMemcpyHostToDevice));
    ...
    KC_FP_TYPE * lPrior;
    checkCudaErrors(cudaMalloc((void**)&lPrior,sizeof(KC_FP_TYPE)*(NH+1)));
    checkCudaErrors(cudaMemcpy(lPrior,(KC_FP_TYPE*)mxGetPr(prhs[10]),sizeof(KC_FP_TYPE)*(NH+1),cudaMemcpyHostToDevice));
    ...
    checkCudaErrors(cudaMalloc((void**)&der_log_p_y,sizeof(KC_FP_TYPE)*(NT)*(NH+1)));    
    KC_FP_TYPE * der_log_p_y_sum;
    checkCudaErrors(cudaMalloc((void**)&der_log_p_y_sum,sizeof(KC_FP_TYPE)*(NH+1)*1));    
    ...
    checkCudaErrors(cudaMalloc((void**)&G_log_p_y1,sizeof(KC_FP_TYPE)*(NT)*(NH+1)*(NH+1)));    
    KC_FP_TYPE * G_log_p_y_sum;
    checkCudaErrors(cudaMalloc((void**)&G_log_p_y_sum,sizeof(KC_FP_TYPE)*(NH+1)*(NH+1)));    
    ...
    kcBoundaryLikelihoodTrialHist<<< numBlocks,blockSize >>>(y,spe,lambda,crossingTimes,trIdx,g,dt, NT,log_p_y,der_log_p_y,G_log_p_y1,h_filt,y_hist,NH);
    ...
    if(nlhs > 1) {
        plhs[1] = mxCreateNumericMatrix(NH+1,1,KC_FP_TYPE_MATLAB,mxREAL);
        checkCudaErrors(cudaMemcpy((KC_FP_TYPE*)mxGetPr(plhs[1]),der_log_p_y_sum,sizeof(KC_FP_TYPE)*(NH+1)*(1),cudaMemcpyDeviceToHost));
    }
    if(nlhs > 2) {
        plhs[2] = mxCreateNumericMatrix(NH+1,NH+1,KC_FP_TYPE_MATLAB,mxREAL);
        checkCudaErrors(cudaMemcpy((KC_FP_TYPE*)mxGetPr(plhs[2]),G_log_p_y_sum,sizeof(KC_FP_TYPE)*(NH+1)*(NH+1),cudaMemcpyDeviceToHost));
    }
