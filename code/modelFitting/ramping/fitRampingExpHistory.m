...
function [ RampFit, RampSamples, LatentDataHandler] = fitRampingExpHistory(timeSeries,params)
...
for ss = 2:totalSamples
    ...
    kcRampPathSamplerExpHist(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,gpu_trBetaIndex,RampSamples.betas(ss-1,:),RampSamples.w2s(ss-1),RampSamples.l_0(ss-1),RampSamples.gammas(ss-1),timeSeries.delta_t, params.rampSampler.numParticles, params.rampSampler.minNumParticles,params.rampSampler.sigMult,maxTrLength, c, p, gpu_spe);
    ...
    [log_p_lambda_h, der_log_p_y_h, H_log_p_y] = kcGammaSpikeHistorySamplerExp(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,RampSamples.gammas(ss-1),gpu_spe,timeSeries.delta_t,reshape(H_prior',[(NH+1)^2 1]),RampSamples.hs(ss-1,:),gpu_spikeHistory,dL_prior);                                                                                                        
    ...
    [log_p_lambda_hstar, der_log_p_y_hstar, H_log_p_y_star] = kcGammaSpikeHistorySamplerExp(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,hs_star(NH+1),gpu_spe_star,timeSeries.delta_t,reshape(H_prior_star',[(NH+1)^2 1]),hs_star(1:NH),gpu_spikeHistory,dL_prior_star);
