...
__device__ KC_FP_TYPE lh(KC_FP_TYPE y, KC_FP_TYPE x, KC_FP_TYPE g, KC_FP_TYPE dt, KC_FP_TYPE bias) {
    KC_FP_TYPE fr = KC_MAX(KC_MIN(KC_MAXN,log1p(exp(x*g))+bias),KC_MINN);
...
__global__ void kcSimGBPaths(const  KC_FP_TYPE * y, const int * trIdx, const int * betaIdx, KC_FP_TYPE * xx, const KC_FP_TYPE * b,const KC_FP_TYPE w2,const  KC_FP_TYPE l_0, const KC_FP_TYPE g, const KC_FP_TYPE dt, KC_FP_TYPE * log_p, const int NT, const int TT,  const int sim, KC_FP_TYPE bias) {
    int idx = blockIdx.x*blockDim.x+threadIdx.x;
    if(idx < NT ) {
        int trNum = idx;
        int T1  = trIdx[trNum];
        ...
        log_p[currIdx] = lh(y[T1],xx[T1],g,dt,bias);
        for(int ii = T1+1; ii < trIdx[trNum+1];ii++) {
            ...
            log_p[currIdx] += lh(y[ii],xx[ii],g,dt,bias);
        }
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    for(int kk = 0; kk < trialsToSim; kk++) {
        ...
        //calculate path + logP
        kcSimGBPaths<<<nBlocks,blockSize>>>(y,trIdx,betaIdx,xx,b_gpu,w,l_0,g,dt,log_p,NT,TT,kk,bias);
