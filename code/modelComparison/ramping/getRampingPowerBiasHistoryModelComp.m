function [WAIC, DIC, l_like, trial_WAIC, trial_DIC, trial_l_like, sample_likelihoods, sample_trial_likelihoods ] = getRampingLog1PPowerBiasMultTrialComp(RampSamples,params,modelFit,timeSeries,pow)
...
[l_like,trial_l_like] = kcRampLikelihoodLog1PPowerBiasMult(gpu_y, gpu_trIndex, gpu_trBetaIndex, gpu_spe, betas, w2s, l_0, gamma, timeSeries.delta_t, params.DIC.meanLikelihoodSamples, bias, pow);
...
for ss = 1:thinRate:params.MCMC.nSamples
    ...
    [sample_likelihoods(ss), sample_trial_likelihoods(:,ss)] = kcRampLikelihoodLog1PPowerBiasMult(gpu_y, gpu_trIndex, gpu_trBetaIndex, gpu_spe, RampSamples.betas(ss+burnIn,:), RampSamples.w2s(ss+burnIn), RampSamples.l_0(ss+burnIn), RampSamples.gammas(ss+burnIn), timeSeries.delta_t, params.DIC.likelihoodSamples, RampSamples.biases(ss+burnIn),pow);
    kcFreeGPUArray(gpu_spe);
end
