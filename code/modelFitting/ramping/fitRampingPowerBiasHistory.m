...
function [ RampFit, RampSamples, LatentDataHandler] = fitRampingPowerBiasHistory(timeSeries,params,pow)
...
for ss = 2:totalSamples
    ...
    kcRampPathSamplerLog1PPowerBiasMult(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,gpu_trBetaIndex,RampSamples.betas(ss-1,:),RampSamples.w2s(ss-1),RampSamples.l_0(ss-1),RampSamples.gammas(ss-1),timeSeries.delta_t, params.rampSampler.numParticles, params.rampSampler.minNumParticles,params.rampSampler.sigMult,maxTrLength, c, p, gpu_spe, RampSamples.biases(ss-1), pow);
...
%         g_delta = params.rampSampler.epsilon_fixed;
    ...
    [log_p_lambda_h, der_log_p_y_h, H_log_p_y] = kcGammaSpikeHistorySamplerLog1PPowerBiasMult(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,RampSamples.gammas(ss-1),gpu_spe,timeSeries.delta_t,reshape(H_prior',[(NH+2)^2 1]),RampSamples.hs(ss-1,:),gpu_spikeHistory,dL_prior,RampSamples.biases(ss-1),pow);                                                                                                        
    ...
    if hs_star(NH+1) > 0 && hs_star(NH+2) > 0
        ...
        [log_p_lambda_hstar, der_log_p_y_hstar, H_log_p_y_star] = kcGammaSpikeHistorySamplerLog1PPowerBiasMult(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,hs_star(NH+1),gpu_spe_star,timeSeries.delta_t,reshape(H_prior_star',[(NH+2)^2 1]),hs_star(1:NH),gpu_spikeHistory,dL_prior_star,hs_star(NH+2),pow);
    ...
    if ss > params.MCMC.burnIn && mod(ss-params.MCMC.burnIn-1,params.MCMC.thinRate)==0;
        ...
        fr = (log1p(exp(min(lambdaN,1)*RampSamples.gammas(ss))).^pow+RampSamples.biases(ss)).*exp(spe);
