...
function [ RampFit, RampSamples, LatentDataHandler] = fitRampingSoftplusHistory(timeSeries,params)
...
%% initial values
RampSamples.betas(1,:) = sort(mvnrnd(zeros(NC,1),0.001*eye(NC)),'ascend'); % originally 0
...
%         g_delta = params.rampSampler.epsilon_fixed;
    ...
    [log_p_lambda_h, der_log_p_y_h, H_log_p_y] = kcGammaSpikeHistorySamplerLog1PMult(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,RampSamples.gammas(ss-1),gpu_spe,timeSeries.delta_t,reshape(H_prior',[(NH+1)^2 1]),RampSamples.hs(ss-1,:),gpu_spikeHistory,dL_prior);                                                                                                            
    ...
    [log_p_lambda_hstar, der_log_p_y_hstar, H_log_p_y_star] = kcGammaSpikeHistorySamplerLog1PMult(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,hs_star(NH+1),gpu_spe_star,timeSeries.delta_t,reshape(H_prior_star',[(NH+1)^2 1]),hs_star(1:NH),gpu_spikeHistory,dL_prior_star);
    ...
    if ss > params.MCMC.burnIn && mod(ss-params.MCMC.burnIn-1,params.MCMC.thinRate)==0;
        ...
        fr = log1p(exp(min(lambdaN,1)*RampSamples.gammas(ss))).*exp(spe);
