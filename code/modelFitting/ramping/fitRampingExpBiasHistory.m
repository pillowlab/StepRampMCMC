...
function [ RampFit, RampSamples, LatentDataHandler] = fitRampingExpBiasHistory(timeSeries,params)
...
initialGamma = min(initialGamma,5); %keep initial gamma within some bounds
...
%% initial values
RampSamples.betas(1,:) = sort(mvnrnd(zeros(NC,1),(1e-4)^2*eye(NC)),'ascend'); % 0
RampSamples.w2s(1,:)   = 1e-4 + 9e-4*rand(1,1); % 0.005
RampSamples.l_0(1)     = max(0.1,min(0.9, normrnd(log(startFR)/initialGamma,0.01)));%0.5; % l_0 = exp(x_0 g) , x_0 = log(l_0) / g
RampSamples.gammas(1)  = max(1e-20,normrnd(initialGamma,0.1));
RampSamples.biases(1)  = max(0.1,normrnd(1,0.1));
...
for ss = 2:totalSamples
    ...
    kcRampPathSamplerExpHistBias(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,gpu_trBetaIndex,RampSamples.betas(ss-1,:),RampSamples.w2s(ss-1),RampSamples.l_0(ss-1),RampSamples.gammas(ss-1),timeSeries.delta_t, params.rampSampler.numParticles, params.rampSampler.minNumParticles,params.rampSampler.sigMult,maxTrLength, c, p, gpu_spe, RampSamples.biases(ss-1));
...
%         g_delta = params.rampSampler.epsilon_fixed;
    ...
    dL_prior = [-(RampSamples.hs(ss-1,:)'-h_mu)/h_sig2; (gamma_a-1)/RampSamples.gammas(ss-1)-gamma_b; (bias_a-1)/RampSamples.biases(ss-1) - bias_b];
    H_prior = diag([1/h_sig2*ones(NH,1); (gamma_a-1)/RampSamples.gammas(ss-1)^2; (bias_a-1)/RampSamples.biases(ss-1)^2]);
    
    % compute log prob, gradients, hessian for current sampled values
    [log_p_lambda_h, der_log_p_y_h, H_log_p_y] = kcGammaSpikeHistorySamplerExpBias(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,RampSamples.gammas(ss-1),gpu_spe,timeSeries.delta_t,reshape(H_prior',[(NH+2)^2 1]),RampSamples.hs(ss-1,:),gpu_spikeHistory,dL_prior,RampSamples.biases(ss-1));                                                                                                        
    ...
    hp_mu = [RampSamples.hs(ss-1,:)'; RampSamples.gammas(ss-1); RampSamples.biases(ss-1)] + 1/2*g_delta.^2.*( (H_log_p_y) \ (der_log_p_y_h) );
    ...
    if hs_star(NH+1) > 0 && hs_star(NH+2) > 0
        
        % compute log probability of sampled values under the proposal
        % distribution
        log_qh_star = -(NH+2)/2*log(2*pi)-1/2*log(det(hp_sig))-1/2*((hs_star(:)-hp_mu(:))'/hp_sig*(hs_star(:)-hp_mu(:)));
        
        % derivatives of log priors of sampled hs and gamma
        dL_prior_star = [-(hs_star(1:NH)-h_mu)/h_sig2; (gamma_a-1)/hs_star(NH+1)-gamma_b; (bias_a-1)/hs_star(NH+2) - bias_b];
        H_prior_star = diag([1/h_sig2*ones(NH,1); (gamma_a-1)/hs_star(NH+1)^2; (bias_a-1)/hs_star(NH+2)^2]);
        
        % compute log prob, gradients, hessian for new sampled values
        [log_p_lambda_hstar, der_log_p_y_hstar, H_log_p_y_star] = kcGammaSpikeHistorySamplerExpBias(gpu_lambdaN,gpu_auxThresholdN,gpu_y,gpu_trIndex,hs_star(NH+1),gpu_spe_star,timeSeries.delta_t,reshape(H_prior_star',[(NH+2)^2 1]),hs_star(1:NH),gpu_spikeHistory,dL_prior_star,hs_star(NH+2));
        ...
        % distribution
        log_qh = -(NH+2)/2*log(2*pi)-1/2*log(det(hp_sig_star))-1/2*(([RampSamples.hs(ss-1,:)'; RampSamples.gammas(ss-1); RampSamples.biases(ss-1)] - hp_mu_star(:))'/hp_sig_star*([RampSamples.hs(ss-1,:)'; RampSamples.gammas(ss-1); RampSamples.biases(ss-1)] -hp_mu_star(:)));
        
        % compute probabilities
        log_p_bias = bias_a*log(bias_b) - gammaln(bias_a) + (bias_a-1) * log(RampSamples.biases(ss-1))    - bias_b*RampSamples.biases(ss-1);
        log_p_bias_star = bias_a*log(bias_b) - gammaln(bias_a) + (bias_a-1) * log(hs_star(NH+2))    - bias_b*hs_star(NH+2);
        log_ph = log_p_lambda_h + sum(-1/2*log(2*pi) - 1/2*log(h_sig2) - ( (RampSamples.hs(ss-1,:)'-h_mu).^2 ./ (2*h_sig2) ))+ gamma_a*log(gamma_b) - gammaln(gamma_a) + (gamma_a-1) * log(RampSamples.gammas(ss-1))    - gamma_b*RampSamples.gammas(ss-1) + log_p_bias;
        log_ph_star = log_p_lambda_hstar + sum(-1/2*log(2*pi) - 1/2*log(h_sig2) - ( (hs_star(1:NH) - h_mu).^2 ./ (2*h_sig2) ))+gamma_a*log(gamma_b) - gammaln(gamma_a) + (gamma_a-1) * log(hs_star(NH+1)) - gamma_b*hs_star(NH+1) + log_p_bias_star;
    ...
    if(sum(imag(hs_star))==0 && hs_star(NH+1) > 0 && hs_star(NH+2) > 0 && lrand < log_a)
        RampSamples.hs(ss,:) = hs_star(1:NH);
        RampSamples.gammas(ss) = hs_star(NH+1);
        RampSamples.biases(ss) = hs_star(NH+2);
