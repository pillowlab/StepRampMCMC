...
__global__ void kcSimGBPaths(const  KC_FP_TYPE * y, const int * trIdx, const int * betaIdx, KC_FP_TYPE * xx, const KC_FP_TYPE * b,const KC_FP_TYPE w2,const  KC_FP_TYPE l_0, KC_FP_TYPE * g, const KC_FP_TYPE dt, KC_FP_TYPE * log_p, const int NT, const int TT,  const int sim, KC_FP_TYPE * spe, int numNeur, KC_FP_TYPE * bias) {
    int idx = blockIdx.x*blockDim.x+threadIdx.x;
    if(idx < NT ) {
        int trNum = idx;
        int T1  = trIdx[trNum];
        ...
        for(int nn = 0; nn < numNeur; nn++) {
            log_p[currIdx] += lh(y[T1+nn*TT],xx[T1],g[nn],dt,spe[T1+nn*TT],bias[nn]);
        }
        for(int ii = T1+1; ii < trIdx[trNum+1];ii++) {
            ...
            for(int nn = 0; nn < numNeur; nn++) {
                log_p[currIdx] += lh(y[ii+nn*TT],xx[ii],g[nn],dt,spe[ii+nn*TT],bias[nn]);
            }
        }
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    for(int kk = 0; kk < trialsToSim; kk++) {
        ...
        //calculate path + logP
        kcSimGBPaths<<<nBlocks,blockSize>>>(y,trIdx,betaIdx,xx,b_gpu,w,l_0,g,dt,log_p,NT,TT,kk,spe,numNeur,bias);
