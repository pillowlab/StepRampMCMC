...
__global__ void kcMoveParticles(KC_FP_TYPE * y, KC_FP_TYPE * pos, KC_FP_TYPE * wt,  KC_FP_TYPE * b, int * betaIdxVector, KC_FP_TYPE l_0, KC_FP_TYPE g, KC_FP_TYPE w, KC_FP_TYPE dt, KC_FP_TYPE * randN, KC_FP_TYPE sigMult, KC_FP_TYPE * log_li, KC_FP_TYPE * lw, KC_FP_TYPE * lw2, KC_FP_TYPE * ncdf, KC_FP_TYPE * posc, int * trIdx, int NT, int TT, int numParticles, int t) {
    ...
    if(tr_num < NT) {
        ...
        if(t < trLength) {
            ...
            KC_FP_TYPE log_pi_k = -log(maxI)-0.5*(log(2.0*M_PI*sig2) + (dpos*dpos)/sig2);
            
            //to be stored for each particle: ncdf, lw, lw2
            ncdf[idx]     = normcdf((1.0-mup)/sw); 
            
            KC_FP_TYPE dposp = pos[idx]-mup;
            KC_FP_TYPE log_p  =  -0.5*(log(2.0*M_PI*w) +  (dposp*dposp)/w);//-0*log(maxI)
            //log_li[pidx]  = -h(pos[idx],g,dt)+y[row]*(log(fmax(h(pos[idx],g,1.0),1e-30))+log(dt))-lgamma(y[row]+1.0);
            
            KC_FP_TYPE h_c = h(pos[idx],g,1.0);
            log_li[pidx]  = -h_c*dt+y[row]*(log(fmax(h_c,1e-30)*dt))-lgamma(y[row]+1.0);
            
            KC_FP_TYPE pw = (t==0)?(log(1.0/(KC_FP_TYPE)numParticles) ):( log(fmax(wt[idx-1], 1e-30)) );
...
__global__ void kcSetupLG(KC_FP_TYPE * y,KC_FP_TYPE * lg,KC_FP_TYPE g, KC_FP_TYPE dt,int TT) {
    ...
    if(idx < TT) {
        double h_c = h(1.0,g, dt);
        lg[idx] = exp( -h_c + y[idx]*log(fmax(h_c,1e-30)) - lgamma(y[idx]+1.0));
    }
}
        ...
        if(t < trLength) {
            ...
            for(int p_num = 0; p_num < numParticles; p_num++) {
                ...
                if(nEff[tr_num] < minEffParticles && t-1==trLength) {
                    cumsum[pidx] = 1.0/(KC_FP_TYPE)numParticles*(1.0+p_num);
                }
            }
    ...
    if(tr_num < NT) {
        ...
        for(int t = 0; t < trLength; t++) {
            if(t == crossingTimes[tr_num]) {
                ...
                KC_FP_TYPE minS = normcdf((1-mu)/sw);
                if(minS >= 1.0-1e-5) {
                    lambda[t_0 + t] = 1.0;
                }
                else {
                    lambda[t_0 + t] = mu+sw*normcdfinv( minS + (1.0-minS)*randUni[t_0+t]);
                }
            }
            else if(t > crossingTimes[tr_num]) {
                lambda[t_0 + t] = lambda[t_0 + t - 1] + cb + sw*normcdfinv( randUni[t_0+t]);
            }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    int blockSizeN2 = 32;//1
    int nBlocksN2   = NT/blockSizeN2 + ((NT%blockSizeN2==0)?0:1);
    
    int blockSizeN3 = 2;//1
    int nBlocksN3   = NT/blockSizeN3 + ((NT%blockSizeN3==0)?0:1);
    ...
    int blockSize2 = 32;//8
    int nBlocks2   = totalThreads/blockSize2 + ((totalThreads%blockSize2==0)?0:1);
    ...
    for (int ii = 0; ii < maxTrialLength;ii++) {
        ...
        kcResampleParticles <<< nBlocks2, blockSize2 >>> (y,pos,posc,wt,log_li,wt_p, minEffParticles,cumsum,nEff,randN,p_cet_0,p_cgt_0a,p_cgt_0b,ncdf,trIdx, NT, TT, numParticles, ii); 
        ce = cudaDeviceSynchronize();
        if(ce != cudaSuccess) {
        ...
        kcPropogateBoundaryDensity <<< nBlocksN2,blockSizeN2 >>> (y,p_clt,p_cet,p_cgt,p_clte,p_cpr,p_cet_0,p_cgt_0a, p_cgt_0b, lg, nEff, minEffParticles, cumsum,trIdx, NT, TT, numParticles, ii);
        ce = cudaDeviceSynchronize();
        if(ce != cudaSuccess) {
