...
__device__ KC_FP_TYPE h(KC_FP_TYPE z, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE bias,KC_FP_TYPE steep,KC_FP_TYPE center) {
    KC_FP_TYPE fr = KC_MAX(KC_MIN( KC_MAXN, gamma / ( 1 + KC_EXP(-steep*(z-center)) ) + bias ),KC_MINN);
    return fr*dt;
}


//one thread per particle <<< nTrials,nParticles >>>
__global__ void kcMoveParticles(KC_FP_TYPE * y, KC_FP_TYPE * pos, KC_FP_TYPE * wt,  KC_FP_TYPE * b, int * betaIdxVector, KC_FP_TYPE l_0, KC_FP_TYPE g, KC_FP_TYPE w, KC_FP_TYPE dt, KC_FP_TYPE * randN, KC_FP_TYPE sigMult, KC_FP_TYPE * log_li, KC_FP_TYPE * lw, KC_FP_TYPE * lw2, KC_FP_TYPE * ncdf, KC_FP_TYPE * posc, int * trIdx, int NT, int TT, int numParticles, int t, KC_FP_TYPE bias,KC_FP_TYPE steep,KC_FP_TYPE center) {
    ...
    if(tr_num < NT) {
        ...
        if(t < trLength) {
            ...
            log_li[pidx]  = -h(pos[idx],g,dt,bias,steep,center)+y[row]*(log(fmax(h(pos[idx],g,1.0,bias,steep,center),1e-30))+log(dt))-lgamma(y[row]+1);
...
__global__ void kcSetupLG(KC_FP_TYPE * y,KC_FP_TYPE * lg,KC_FP_TYPE g, KC_FP_TYPE dt,int TT, KC_FP_TYPE bias,KC_FP_TYPE steep,KC_FP_TYPE center) {
    int idx = blockIdx.x*blockDim.x + threadIdx.x;
    if(idx < TT) {
        lg[idx] = exp( -h(1,g, dt,bias,steep,center) + y[idx]*log(fmax(h(1,g,dt,bias,steep,center),1e-30)) - lgamma(y[idx]+1));
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    KC_FP_TYPE  steep     = mxGetScalar(prhs[17]);
    KC_FP_TYPE  center     = mxGetScalar(prhs[18]);
    ...
    //__global__ void kcSetupLG(KC_FP_TYPE * y,KC_FP_TYPE * lg,KC_FP_TYPE g, KC_FP_TYPE dt,int TT,KC_FP_TYPE bias,KC_FP_TYPE steep,KC_FP_TYPE center) {
    kcSetupLG <<< nBlocksT, blockSizeT >>> (y,lg,g,dt,TT,bias,steep,center);
    ce = cudaDeviceSynchronize();
    if(ce != cudaSuccess) {
    ...
    for (int ii = 0; ii < maxTrialLength;ii++) {
        ...
        kcMoveParticles <<< nBlocks, blockSize >>> (y,pos,wt, b_gpu,betaIdxVector,l_0,g,w,dt,randN, sigMult,log_li,lw,lw2,ncdf, posc, trIdx, NT, TT, numParticles, ii, bias,steep,center);
        ce = cudaDeviceSynchronize();
        if(ce != cudaSuccess) {
            int currDev;
            cudaGetDevice(&currDev);
