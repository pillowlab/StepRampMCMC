...
% modelFit.phi.mean
% modelFit.r.mean
function [l_like, trial_likelihood] = getSteppingLogLikelihoodGLMMult(timeSeries,params,modelFit,spe)
...
fr1 = modelFit.alpha.mean(1)*exp(spe);                                                                                     
fr2 = modelFit.alpha.mean(2)*exp(spe);
fr3 = modelFit.alpha.mean(3)*exp(spe);
...
for tr = 1:NT
    ...
    ps1 = -timeSeries.delta_t*fr1(T1:T2)+timeSeries.y(T1:T2)   .*(log(fr1(T1:T2)) + log(timeSeries.delta_t)) - gammaln(timeSeries.y(T1:T2)+1);
    ps2 = -timeSeries.delta_t*fr2(T2:-1:T1)+timeSeries.y(T2:-1:T1).*(log(fr2(T2:-1:T1)) + log(timeSeries.delta_t)) - gammaln(timeSeries.y(T2:-1:T1)+1);
    ps3 = -timeSeries.delta_t*fr3(T2:-1:T1)+timeSeries.y(T2:-1:T1).*(log(fr3(T2:-1:T1)) + log(timeSeries.delta_t)) - gammaln(timeSeries.y(T2:-1:T1)+1);
