...
__global__ void kcMoveParticles(KC_FP_TYPE * y, KC_FP_TYPE * spe, KC_FP_TYPE * pos, KC_FP_TYPE * wt,  KC_FP_TYPE * b, int * betaIdxVector, KC_FP_TYPE l_0, KC_FP_TYPE g, KC_FP_TYPE w, KC_FP_TYPE dt, KC_FP_TYPE * randN, KC_FP_TYPE sigMult, KC_FP_TYPE * log_li, KC_FP_TYPE * lw, KC_FP_TYPE * lw2, KC_FP_TYPE * ncdf, KC_FP_TYPE * posc, int * trIdx, int NT, int TT, int numParticles, int t, KC_FP_TYPE log_power) {
    ...
    if(tr_num < NT) {
        ...
        if(t < trLength) {
            ...
            log_li[pidx]  = -h(pos[idx],g,dt,spe[row],log_power)+y[row]*(log(fmax(h(pos[idx],g,1.0,spe[row],log_power),1e-30))+log(dt))-lgamma(y[row]+1);
...
__global__ void kcSetupLG(KC_FP_TYPE * y,KC_FP_TYPE * spe,KC_FP_TYPE * lg,KC_FP_TYPE g, KC_FP_TYPE dt,int TT, KC_FP_TYPE log_power) {
    int idx = blockIdx.x*blockDim.x + threadIdx.x;
    if(idx < TT) {
        lg[idx] = exp( -h(1,g, dt,spe[idx],log_power) + y[idx]*log(fmax(h(1,g,dt,spe[idx],log_power),1e-30)) - lgamma(y[idx]+1));
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    KC_FP_TYPE  log_power     = mxGetScalar(prhs[17]);
    ...
    kcSetupLG <<< nBlocksT, blockSizeT >>> (y,spe,lg,g,dt,TT,log_power);
    ce = cudaDeviceSynchronize();
    if(ce != cudaSuccess) {
    ...
    for (int ii = 0; ii < maxTrialLength;ii++) {
        ...
        kcMoveParticles <<< nBlocks, blockSize >>> (y,spe,pos,wt, b_gpu,betaIdxVector,l_0,g,w,dt,randN, sigMult,log_li,lw,lw2,ncdf, posc, trIdx, NT, TT, numParticles, ii,log_power);
        ce = cudaDeviceSynchronize();
        if(ce != cudaSuccess) {
            int currDev;
            cudaGetDevice(&currDev);
