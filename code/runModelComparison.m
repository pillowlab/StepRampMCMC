...
[diffToBoundSamples, ~, acceptedCountAR1B, final_g_delta]    = fitGaussianBoundModelGPUAux(timeSeries,params,MCMCsamples,particleParams,DiffToBoundFitInit,cellNum); %#ok<ASGLU>

diffusionToBoundFit = true;

particleParams.g_delta = final_g_delta;

sampleSummaryDiffToBound.meanBeta  = diffToBoundSamples.meanBeta;
sampleSummaryDiffToBound.meanW2    = diffToBoundSamples.meanW2;
sampleSummaryDiffToBound.meanL0    = diffToBoundSamples.meanL0;
sampleSummaryDiffToBound.meanGamma = diffToBoundSamples.meanGamma;
sampleSummaryDiffToBound.mean = diffToBoundSamples.mean;

sampleSummaryDiffToBound.intervalBeta  = prctile(diffToBoundSamples.betas(((burnInTime(2)+1):params.thinRate(1):end),:),[2.5 97.5],1);
sampleSummaryDiffToBound.intervalW2    = prctile(diffToBoundSamples.w2s((burnInTime(2)+1):params.thinRate(1):end,:),[2.5 97.5],1);
sampleSummaryDiffToBound.intervalL0    = prctile(diffToBoundSamples.l_0((burnInTime(2)+1):params.thinRate(1):end),[2.5 97.5],1);
sampleSummaryDiffToBound.intervalGamma = prctile(diffToBoundSamples.gammas((burnInTime(2)+1):params.thinRate(1):end),[2.5 97.5],1);

particleParams2 = particleParams;
particleParams2.numParticles = 400;
particleParams2.minNumParticles = 40;

% params.gamma_1 = 1;%2.5;   %%==================================================================================================================
% params.gamma_2 = 0.01;%0.005;

if(cellNum > 200 && cellNum < 300)
    [~, ~,  trueParams] = loadCellResults(cellNum-200, '', 'Results/Main/');
else
    trueParams = [];
end


[modelComparison(1,2), modelComparison(2,2), modelComparison(3,2), modelComparison(4,2),modelComparison(5,2), modelComparison(6,2), diffToBound_MAP_ML]  = bayesFactorsGaussianBoundAux(diffToBoundSamples,params,particleParams2,timeSeries,likelihoodMCSamples,DICsamples,timeSeriesTest,trueParams);

diffToBoundSamples.lambdas = [];
clearLatentsDB();
modelComparisonB = modelComparison(:,2);

save([cellFilename '_diffToBound_summary.mat'],'cellNum', 'sampleSummaryDiffToBound','modelComparisonB','modelComparisonIndex','diffToBound_MAP_ML','params','-v7.3');
save([cellSamplesFilename '_diffToBound_samples.mat'],'diffToBoundSamples','-v7.3');
clear sampleSummaryDiffToBound
%save([cellFilename '_diffToBound_samples.mat'], '-struct', 'diffToBoundSamples','-v7.3');
clear diffToBoundSamples
