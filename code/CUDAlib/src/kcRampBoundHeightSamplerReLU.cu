...
__device__ KC_FP_TYPE h(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE bias) {
    KC_FP_TYPE fr = KC_MIN(KC_MAXN,KC_MAX(lambda,bias)*gamma);
    return fr*dt;
}

__device__ KC_FP_TYPE dhg(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt,KC_FP_TYPE bias) {
    return dt*KC_MAX(bias,lambda);
}

__device__ KC_FP_TYPE dhb(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt,KC_FP_TYPE bias) {
    if(lambda > bias) {
        return 0;
    }
    else {
...
__global__ void kcBoundaryLikelihoodTrial(KC_FP_TYPE * y, KC_FP_TYPE * lambdas, int * crossingTimes, int * mBlkIdx, KC_FP_TYPE g, KC_FP_TYPE dt, int NT, KC_FP_TYPE * llSum, KC_FP_TYPE * trialSum, KC_FP_TYPE * trialSumRiemann, KC_FP_TYPE bias, KC_FP_TYPE * trialSumB, KC_FP_TYPE * trialSumRiemannB, KC_FP_TYPE * trialRiemannOffDiag) {
    ...
    if(idx < NT) {
        ...
        for(int ii = mBlkIdx[idx]; ii < mBlkIdx[idx+1]; ii++)  {
            ...
            KC_FP_TYPE drg = dhg(trueLambda,g,1,bias);
            KC_FP_TYPE drb = dhb(trueLambda,g,1,bias);

            trialSum[idx] += (y[ii]/r-dt)*drg;
            trialSumRiemann[idx] += -1*dt*drg*drg/r;
            trialSumB[idx] += (y[ii]/r-dt)*drb;
            trialSumRiemannB[idx] += -1*dt*drb*drb/r;
            trialRiemannOffDiag[idx] += -1*dt*drg*drb/r;
        }
    }
}
