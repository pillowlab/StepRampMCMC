...
__global__ void kcComputeLogPY(KC_FP_TYPE * lp_y, const KC_FP_TYPE * y, const KC_FP_TYPE * alphas,  const KC_FP_TYPE dt, const int TT) {
    int tt = blockIdx.x*blockDim.x+threadIdx.x;
    if(tt < TT) {
        for(int ii = 0; ii < 3; ii++) { 
            lp_y[tt+ii*TT] = log_p_y(y[tt],alphas[ii],dt);
        }
    }
}
 ...
 */
__global__ void kcSampleSMStates(KC_FP_TYPE * z, KC_FP_TYPE * s, KC_FP_TYPE * sampleStats, KC_FP_TYPE * y, KC_FP_TYPE * lp_y, int * trialIndex, int * trialCoh, int NT, KC_FP_TYPE * alphas, KC_FP_TYPE * phi, KC_FP_TYPE delta_t, int maxJump, KC_FP_TYPE * randU, KC_FP_TYPE * nbPDF, KC_FP_TYPE * jumpToProbs, const int TT) {
    ...
    if(idx < NT) {
        ...
        for(int ii = T-1; ii >= 0; ii--) {
            ...
            jumpToProbs[jumpT1_2+ii] = ((ii < T-1)?(jumpToProbs[jumpT1_2+ii+1]):(0)) + lp_y[T1+ii+TT];//log_p_y(y[T1+ii],alphas[1],delta_t) ;
            jumpToProbs[jumpT1_3+ii] = ((ii < T-1)?(jumpToProbs[jumpT1_3+ii+1]):(0)) + lp_y[T1+ii+TT*2];//log_p_y(y[T1+ii],alphas[2],delta_t) ;
        }
        ...
        for(int ii = 0; ii < maxJump; ii++) {
            ...
            if(ii < T) {
                KC_FP_TYPE p_y_init = lp_y[T1+ii];
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    KC_FP_TYPE * lp_y;
    checkCudaErrors(cudaMalloc((void**)&lp_y,sizeof(KC_FP_TYPE)*TT*3)); 
    ...
    int blockSizeN;
    blockSize = 128;
    nBlocks   = TT/blockSize + ((TT%blockSize==0)?0:1);
    blockSizeN = 4;
    nBlocksN   = NT/blockSizeN + ((NT%blockSizeN==0)?0:1);
    
    kcComputeLogPY<<<nBlocks,blockSize>>>(lp_y, y, alphas,  dt, TT);
    
    kcSampleSMStates<<<nBlocksN,blockSizeN>>>(z, s, sampleStats,  y, lp_y, trIndex, cohIndex, NT, alphas,  phi, dt, maxJump, randU, nbPDF, calculationSpace,TT);
