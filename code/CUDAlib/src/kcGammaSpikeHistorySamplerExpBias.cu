...
__device__ KC_FP_TYPE h(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE sh, KC_FP_TYPE bias) {
    KC_FP_TYPE fr = KC_MAX(KC_MIN( (KC_EXP( gamma*lambda ) + bias)*KC_EXP(sh) ,KC_MAXN),KC_MINN)*dt;
    return fr;
}

__device__ KC_FP_TYPE dhg(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE sh, KC_FP_TYPE yh) {
    KC_FP_TYPE ex = KC_MAX(KC_MIN(KC_EXP( gamma*lambda + sh),KC_MAXN),KC_MINN);
    return dt*yh*ex;
}

__device__ KC_FP_TYPE dhs(KC_FP_TYPE lambda, KC_FP_TYPE gamma, KC_FP_TYPE dt, KC_FP_TYPE sh, KC_FP_TYPE yh, KC_FP_TYPE bias) {
    KC_FP_TYPE fr = KC_MAX(KC_MIN( (KC_EXP( gamma*lambda ) + bias)*KC_EXP(sh) ,KC_MAXN),KC_MINN);
    return dt*yh*fr;
}


// computes log p(single trial | gamma, fixed lambdas, spike history)
__global__ void kcBoundaryLikelihoodTrialHist(KC_FP_TYPE * y, KC_FP_TYPE * spe, KC_FP_TYPE * lambdas, int * crossingTimes, int * mBlkIdx, KC_FP_TYPE g, KC_FP_TYPE dt, int NT, KC_FP_TYPE * llSum, KC_FP_TYPE * trialSum, KC_FP_TYPE * trialSumRiemann, KC_FP_TYPE * h_filt, KC_FP_TYPE * y_hist, int NH, KC_FP_TYPE bias) {
    ...
    if(idx < NT) {
        ...
        for(int ii = mBlkIdx[idx]; ii < mBlkIdx[idx+1]; ii++)  {
            ...
            //KC_FP_TYPE r    = KC_MAX(KC_MIN(KC_EXP( g*trueLambda + sh)+bias,KC_MAXN),KC_MINN);
            KC_FP_TYPE r    = KC_MAX(KC_MINN,KC_MIN(KC_MAXN, (KC_EXP(g*trueLambda)+bias)*KC_EXP(sh) ));
	    ...
	    for(int jj = 0; jj < NH+2 ; jj++) {
            ...
            if(jj < NH && ii<(mBlkIdx[idx]+jj+1)) {
                ...
                dr = dhs(trueLambda,g,1,sh,yh1,bias);
            }
            else if(jj < NH) {
                yh1 = y[ii-jj-1];
                dr = dhs(trueLambda,g,1,sh,yh1,bias);
                ...
                for(int kk = 0; kk < NH+2; kk++) {
            		...
            		// in that case, we want the ii - jj spike of the y history
               	    if(kk < NH && ii<(mBlkIdx[idx]+kk+1)) {
                        ...
                        dr2 = dhs(trueLambda,g,1,sh,yh2,bias);
            		}
                    else if(kk < NH) {
               	  		yh2 = y[ii-kk-1];
                        dr2 = dhs(trueLambda,g,1,sh,yh2,bias);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    kcBoundaryLikelihoodTrialHist<<< numBlocks,blockSize >>>(y,spe,lambda,crossingTimes,trIdx,g,dt, NT,log_p_y,der_log_p_y,G_log_p_y1,h_filt,y_hist,NH,bias);
